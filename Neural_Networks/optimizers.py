...
class SGD():
    ...
    Methods:
        optimzeGrad (self, x, input, weights, bias, activation, out_delta, lr): Performs the backward pass using SGD optmization method.

    Usage:
        Used to train a fully connected layer through Stochastic Gradient Descent.
    '''
    ## Initializes the SGD function on the CPU.
    def __init__(self):
        '''
        Initializes the parameters for SGD function.
    ...
    def optimizeGrad(self, x, input, weights, bias, activation, out_delta, lr):
        '''
        ...
        nabla_activation = activation.activateGrad(x)
